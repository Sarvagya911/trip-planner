...
Places (hotels, plus fuel/food for stops) are NOT travel segments — they're
things attached to a destination or a route point. So they live in their own
models and ride alongside the trip, keeping the Segment schema clean and
mode-agnostic (the rule that's kept everything extensible).

Same metasearch pattern as the rest of the app: real data shown in-app,
booking/finding happens on an external platform via `book_external_url`.
"""
...
class Place(BaseModel):
    name: str
    category: str = "hotel"          # "hotel" | "fuel" | "food" | ...
    ...
    why: str | None = None
    # Where the user actually books/finds — external, per the metasearch pattern.
